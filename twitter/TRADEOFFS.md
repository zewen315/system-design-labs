...
Not yet done: the domain (`twitter.zewenw.com`) isn't pointed at the droplet yet, so the site is only running over plain HTTP on the droplet's IP for now. TLS (Let's Encrypt via certbot) is the next step once DNS propagates — `nginx.prod.conf` already has `server_name twitter.zewenw.com` set in anticipation of that.
